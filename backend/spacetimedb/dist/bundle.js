...
//#endregion
//#region src/index.ts
const storage = table({ public: true }, {
	owner_key: t.string().primaryKey(),
	value: t.string()
});
const spacetimedb = schema({
	person: table({ public: false }, {
...
const signup = spacetimedb.reducer({
	userid: t.string(),
	passhash: t.string()
}, (ctx, { userid, passhash }) => {
...
const change_password = spacetimedb.reducer({
	userid: t.string(),
	passhash: t.string(),
	new_passhash: t.string()
}, (ctx, { userid, passhash, new_passhash }) => {
	let prev = ctx.db.person.userid.find(userid);
	if (!prev || prev.passhash != passhash) throw new Error("Incorrect password");
	ctx.db.person.userid.update({
		userid,
		passhash
	});
});
const mkkey = (owner, key) => owner.replaceAll(":", "_:") + ":" + key;
const setitem = spacetimedb.reducer({
	owner: t.string(),
	passhash: t.string(),
	key: t.string(),
	value: t.string()
	...
	if (ctx.db.storage.owner_key.find(owner_key)) ctx.db.storage.owner_key.update({
		owner_key,
		value
	});
	else ctx.db.storage.insert({
		owner_key,
		value
	});
});
