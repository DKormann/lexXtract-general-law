...
let ok = { tag: "success" };
let err = { tag: "err" };
const signup = spacetimedb.procedure({
	userid: t.string(),
	passhash: t.string()
}, t.enum("SignupResult", ["success", "err"]), (ctx, { userid, passhash }) => ctx.withTx((c) => {
...
const changePassword = spacetimedb.procedure({
	userid: t.string(),
	passhash: t.string(),
	newPasshash: t.string()
}, t.enum("changePasswordResult", ["success", "err"]), (ctx, { userid, passhash, newPasshash }) => ctx.withTx((c) => {
	let prev = c.db.person.userid.find(userid);
	if (!prev || prev.passhash != passhash) return err;
	c.db.person.userid.update({
		userid,
...
const setitem = spacetimedb.procedure({
	owner: t.string(),
	passhash: t.string(),
	key: t.string(),
	value: t.string()
}, t.enum("setResult", ["success", "err"]), (ctx, { owner, passhash, key, value }) => ctx.withTx((c) => {
	let prev = c.db.person.userid.find(owner);
	if (!prev) return err;
	if (prev.passhash != passhash);
	let owner_key = mkkey(owner, key);
	if (c.db.storage.owner_key.find(owner_key)) c.db.storage.owner_key.update({
		owner_key,
		value
	});
	else c.db.storage.insert({
		owner_key,
		value
	});
	return ok;
